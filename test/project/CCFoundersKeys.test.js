...
const INIT_BASE_URI      = CST.NEW_BASE_URI
...
const TEST_DATA = {
	...
	MINT_OUT : {
		...
		PARAMS : {
			...
			maxSupply_ : INIT_SUPPLY + AIRDROP1 + AIRDROP2 + TOKEN_OWNER_SUPPLY + OTHER_OWNER_SUPPLY + TOKEN_OWNER_CL + OTHER_OWNER_CL + TOKEN_OWNER_WL + OTHER_OWNER_WL,
		},
	},
}
...
async function mintFixture () {
	...
	test_qty       = TEST_DATA.TOKEN_OWNER_SUPPLY
...
async function mintOutFixture () {
	...
	test_amounts = [ 
		TEST_DATA.MINT_OUT.TOKEN_OWNER_CL,
		TEST_DATA.MINT_OUT.OTHER_OWNER_CL,
	]
	await test_contract.connect( test_contract_deployer )
										 .setAnonClaimList( test_accounts, test_amounts )
	...
	test_passMax = TEST_DATA.MINT_OUT.TOKEN_OWNER_WL
	...
	test_passMax = TEST_DATA.MINT_OUT.OTHER_OWNER_WL
			...
			// **************************************
				...
				describe( CONTRACT.METHODS.claimAndStake.SIGNATURE, function () {
					if ( TEST.METHODS.claimAndStake ) {
						...
						it( 'Whitelisted user trying to stake more tokens than claimed should be reverted with ' + CONTRACT.ERRORS.CCFoundersKeys_INSUFFICIENT_KEY_BALANCE, async function () {
							const qty       = 1
							const qtyStaked = qty + 1
							await expect(
								contract.connect( users[ TOKEN_OWNER ] )
												.claimAndStake( qty, qtyStaked )
							).to.be.revertedWith( CONTRACT.ERRORS.CCFoundersKeys_INSUFFICIENT_KEY_BALANCE )
						})
			...
			// **************************************
				...
				describe( CONTRACT.METHODS.mintPreSaleAndStake.SIGNATURE, function () {
					if ( TEST.METHODS.mintPreSaleAndStake ) {
						...
						it( 'Whitelisted user trying to stake more tokens than minted should be reverted with ' + CONTRACT.ERRORS.CCFoundersKeys_INSUFFICIENT_KEY_BALANCE, async function () {
							const passMax   = test_data.TOKEN_OWNER_WL
							const account   = users[ TOKEN_OWNER ].address
							const proof     = generateProof( account, test_data.PASS_ROOT, passMax )
							const pass      = proof.pass
			...
			// **************************************
				...
				describe( CONTRACT.METHODS.mintAndStake.SIGNATURE, function () {
					if ( TEST.METHODS.mintAndStake ) {
						...
						it( 'Trying to stake more tokens than minted should be reverted with ' + CONTRACT.ERRORS.CCFoundersKeys_INSUFFICIENT_KEY_BALANCE, async function () {
							const qty       = 1
							const qtyStaked = qty + 1
							const tx_params = {
								value : test_data.PARAMS.publicMintPrice_.mul( qty )
...
describe( TEST.NAME, function () {
	if ( TEST_ACTIVATION[ TEST.NAME ] ) {
		...
		shouldBehaveLikeCCFoundersKeysAfterMint( mintFixture, TEST_DATA )
